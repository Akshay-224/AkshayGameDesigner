...
// Non-Suspense video texture — renders immediately, video plays when ready
const VideoScreenMesh = ({ textureUrl, geometry, material, ...meshProps }) => {
    ...
    useEffect(() => {
        ...
        // Set texture immediately — THREE.VideoTexture updates itself each frame
        setVideoTexture(texture);
        video.load(); // explicitly trigger CORS-aware load
        video.play().catch(() => {});
...
// Sub-component for image textures (YouTube thumbnails, etc.)
const ImageScreenMesh = ({ textureUrl, geometry, material, ...meshProps }) => {
    const txt = useTexture(textureUrl);
...
const DemoComputer = (props) => {
    const group = useRef()
    const { nodes, materials } = useGLTF(asset('/models/computer.glb'));
    const textureUrl = props.texture || DEFAULT_TEXTURE;
    ...
    return (
        <group ref={group} {...props} dispose={null}>
            <group name="Scene">
                {isVideoUrl(textureUrl)
                    ? <VideoScreenMesh textureUrl={textureUrl} {...screenMeshProps} />
                    : <ImageScreenMesh textureUrl={textureUrl} {...screenMeshProps} />
                }
                <group
                    name="RootNode"
                    position={[0, 1.093, 0]}
